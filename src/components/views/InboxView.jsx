                            ...
                            <div className="divide-y divide-slate-100">
                                {grouped[groupLabel].map(call => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`py-6 transition-all duration-300 ${isExpanded ? 'px-4 -mx-4 rounded-3xl bg-[#F0F7FF] shadow-sm ring-1 ring-[#D1E9FF]/50 my-2' : ''}`}>
                                                {/* --- Call Info Row --- */}
                                                ...
                                                {call.actionItem && (
                                                    <div
                                                        className="mb-3 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full
                                                            bg-blue-50 border border-blue-100/50 
                                                            text-[#007FFF] text-[10px] font-bold tracking-tight
                                                            hover:bg-blue-100 transition-all cursor-pointer group/pill"
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                ...
                                                {isExpanded && (
                                                    <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                                        <div className="p-4 rounded-2xl bg-white border border-[#D1E9FF] shadow-sm">
                                                        ...
                                                        <div className="flex items-center gap-2 mb-2 w-full">
                                                            <button
                                                                ...
                                                                className="bg-[#007FFF] text-white px-5 py-2.5 rounded-full font-bold text-xs flex items-center gap-2 shadow-lg shadow-[#007FFF]/20 active:scale-95 transition-all"
                                                            >
                                                                <Phone size={14} fill="white" /> Call
                                                            </button>
                                                            <button
                                                                onClick={(e) => e.stopPropagation()}
                                                                className="bg-white border border-slate-100 text-slate-700 px-5 py-2.5 rounded-full font-bold text-xs flex items-center gap-2 shadow-sm active:scale-95 transition-all"
                                                            >
                                                        ...
                                                        {/* Audio Player - Premium Music Style */}
                                                        {call.recordingUrl && (
                                                            <div className="bg-white rounded-2xl p-4 flex items-center gap-4 border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]" onClick={(e) => e.stopPropagation()}>
                                                                <button
                                                                    onClick={() => {
                                                                    ...
                                                                    className={`w-11 h-11 rounded-full flex items-center justify-center text-white shadow-lg transition-all active:scale-90 hover:scale-[1.02] ${playingVoiceId === call.id ? 'bg-gradient-to-br from-[#007FFF] to-[#0047AB] shadow-blue-200' : 'bg-gray-900 shadow-gray-200'}`}
                                                                >
                                                                    {playingVoiceId === call.id ? <Pause size={18} fill="white" /> : <Play size={18} fill="white" className="ml-1" />}
                                                                </button>

                                                                <div className="flex-1 flex flex-col gap-1.5">
                                                                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden relative group cursor-pointer"
                                                                        onClick={(e) => {
                                                                            const rect = e.currentTarget.getBoundingClientRect();
                                                                            const percent = (e.clientX - rect.left) / rect.width;
                                                                            const audioEl = document.getElementById(`audio-${call.id}`);
                                                                            if (audioEl) audioEl.currentTime = percent * audioEl.duration;
                                                                        }}
                                                                    >
                                                                        <div
                                                                            className="h-full bg-gradient-to-r from-[#007FFF] to-[#0047AB] rounded-full transition-all duration-75 relative"
                                                                            style={{ width: `${playingVoiceId === call.id ? audioProgress : 0}%` }}
                                                                        >
                                                                            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-md border border-slate-200 translate-x-1/2 scale-0 group-hover:scale-100 transition-transform"></div>
                                                                        </div>
                                                                    </div>
                                                                    <div className="flex justify-between items-center px-0.5">
                                                                        <span className="text-[10px] font-bold text-slate-400 tabular-nums">Playback</span>
                                                                        <span className="text-[10px] font-bold text-slate-400 tabular-nums">
                                                                            {playingVoiceId === call.id ? `${Math.floor(audioProgress)}%` : '0%'}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-full flex justify-between items-center px-2 py-3 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors"
                                                        >
                                                        ...
                                                        {showTranscript && (
                                                            <div className="p-4 bg-white rounded-2xl border border-slate-100 space-y-4 max-h-60 overflow-y-auto no-scrollbar animate-in slide-in-from-top-1">
                                                                {call.transcript ? call.transcript.split(/(?=AI:|Guest:|User:)/g).map((msg, i) => {
                                                                    const isAI = msg.trim().startsWith("AI:");
                                                                    const cleanMsg = msg.replace(/^(AI:|Guest:|User:)/i, '').trim();
                                                                    if (!cleanMsg) return null;
                                                                    return (
                                                                        <div key={i} className={`flex flex-col ${isAI ? 'items-start' : 'items-end'}`}>
                                                                            <span className="text-[9px] font-black tracking-widest uppercase text-slate-300 mb-1">{isAI ? 'Receptionist' : 'Guest'}</span>
                                                                            <div className={`py-2.5 px-4 rounded-2xl text-[13px] leading-relaxed max-w-[85%] ${isAI ? 'bg-slate-50 text-slate-800 rounded-tl-none' : 'bg-[#0047AB] text-white rounded-tr-none'}`}>
                                                                                {msg.replace(/^(AI:|User:)/i, '').trim()}
                                                                            </div>
                                                                        </div>
                                                                    );
                                                                }) : <p className="text-slate-400 text-xs italic text-center">Transcript not available</p>}
                                                            </div>
                                                        )}
                                                    </div>
                                                )}
